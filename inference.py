...
# https://zhuanlan.zhihu.com/p/1894054231067037789
...
MODEL_PATH = "/home/xqzzz1/codes/pytorch-paligemma/paligemma-3b-model"
...
def test_inference(
    ...
    # generate tokens until you see the end token
...
def _sample_top_p(probs: torch.Tensor, p: float):
    ...
    probs_sort[mask] = 0.0
    # redistribute the prob so that they sum up to 1
    probs_sort.div_(probs_sort.sum(dim=-1, keepdim=True))
    # sample a token (its index) from top p distribution
    next_token = torch.multinomial(probs_sort, num_samples=1)
    # get token position in the vocabulary corresponding to the sampled index
